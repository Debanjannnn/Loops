...
let BettingContract = (_dec = NearBindgen({}), _dec2 = call({
  payableFunction: true
}), _dec3 = call({}), _dec4 = view(), _dec5 = view(), _dec6 = view(), _dec(_class = (_class2 = class BettingContract {
  // Use UnorderedMap for efficient storage of user data
  // Maps account IDs (string) to User objects
  users = new UnorderedMap('users_map');
  ...
  place_bet({
    ...
    if (deposit === BigInt(0)) {
      log("Attached deposit must be greater than zero.");
      return;
    }
    log(`${accountId} placed a bet of ${deposit.toString()} yoctoNEAR on ${gameId}`);
    ...
    if (didWin) {
      ...
      log(`${accountId} WON! Received ${winningsAmount.toString()} yoctoNEAR (x${winMultiplier})`);
    } else {
      // User loses - amount goes to contract balance
      user.totalLost += deposit;
      this.contractBalance += deposit;
      log(`${accountId} LOST! ${deposit.toString()} yoctoNEAR added to contract balance`);
    }
  ...
  withdraw() {
    ...
    if (amountToWithdraw > BigInt(0)) {
      log(`${accountId} is attempting to withdraw ${amountToWithdraw.toString()} yoctoNEAR`);
      ...
      const promiseId = promiseBatchCreate(accountId);
      promiseBatchActionTransfer(promiseId, amountToWithdraw);
      log(`${accountId} successfully withdrew ${amountToWithdraw.toString()} yoctoNEAR`);
    } else {
      log(`${accountId} has no withdrawable balance.`);
    }
  }
...
}, _applyDecoratedDescriptor(_class2.prototype, "place_bet", [_dec2], Object.getOwnPropertyDescriptor(_class2.prototype, "place_bet"), _class2.prototype), _applyDecoratedDescriptor(_class2.prototype, "withdraw", [_dec3], Object.getOwnPropertyDescriptor(_class2.prototype, "withdraw"), _class2.prototype), _applyDecoratedDescriptor(_class2.prototype, "get_user_stats", [_dec4], Object.getOwnPropertyDescriptor(_class2.prototype, "get_user_stats"), _class2.prototype), _applyDecoratedDescriptor(_class2.prototype, "get_contract_total_losses", [_dec5], Object.getOwnPropertyDescriptor(_class2.prototype, "get_contract_total_losses"), _class2.prototype), _applyDecoratedDescriptor(_class2.prototype, "get_total_users", [_dec6], Object.getOwnPropertyDescriptor(_class2.prototype, "get_total_users"), _class2.prototype), _class2)) || _class);
function get_total_users() {
  const _state = BettingContract._getState();
  if (!_state && BettingContract._requireInit()) {
    throw new Error("Contract must be initialized");
  }
  const _contract = BettingContract._create();
  if (_state) {
    BettingContract._reconstruct(_contract, _state);
  }
  const _args = BettingContract._getArgs();
  const _result = _contract.get_total_users(_args);
  if (_result !== undefined) if (_result && _result.constructor && _result.constructor.name === "NearPromise") _result.onReturn();else env.value_return(BettingContract._serialize(_result, true));
}
...
function withdraw() {
  ...
  BettingContract._saveToStorage(_contract);
  if (_result !== undefined) if (_result && _result.constructor && _result.constructor.name === "NearPromise") _result.onReturn();else env.value_return(BettingContract._serialize(_result, true));
}
function place_bet() {
  const _state = BettingContract._getState();
...
export { BettingContract, get_contract_total_losses, get_total_users, get_user_stats, place_bet, withdraw };
